...
export function handleActorEvents(socket) {
  Object.values(SERVER_REQUEST_EVENTS.ACTOR).forEach((actorEvent) => {
    socket.on(actorEvent, async (actorId, iosSocketId) => {
      await handleActorEvent(actorEvent, socket, actorId, iosSocketId);
    });
  });
}

export async function handleActorEvent(
  actorEvent,
  socket,
  actorId,
  iosSocketId
  ...
  function createAndEmitActorAbilities(socket, actor, iosSocketId) {
    socket.emit(
      FOUNDRY_EVENTS.ACTOR.ABILITIES,
      {
  ...
  function createAndEmitActorAttributes(socket, actor, iosSocketId) {
    ...
    socket.emit(
      FOUNDRY_EVENTS.ACTOR.ATTRIBUTES,
      {
        deathSaves: {
  ...
  async function createAndEmitActorBaseData(socket, actor, iosSocketId) {
    socket.emit(
      FOUNDRY_EVENTS.ACTOR.BASE_DATA,
      {
        id: actor.data._id,
  ...
  async function createAndEmitActorInventory(socket, actor, iosSocketId) {
    ...
    await Promise.all(
      ...
      socket.emit(
        FOUNDRY_EVENTS.ACTOR.INVENTORY,
        {
  ...
  function createAndEmitActorSpellSlots(socket, actor, iosSocketId) {
    socket.emit(
      FOUNDRY_EVENTS.ACTOR.SPELL_SLOTS,
      {
  ...
  function createAndEmitActorTraits(socket, actor, iosSocketId) {
    let traits = actor.data.data.traits;
    socket.emit(
      FOUNDRY_EVENTS.ACTOR.TRAITS,
      {
  ...
  function createAndEmitActorClasses(socket, actor, iosSocketId) {
    socket.emit(
      FOUNDRY_EVENTS.ACTOR.CLASSES,
      {
