...
export function sendAbilityRoll(socket, abilityRoll) {
    if(abilityRoll) {
        ...
        if (abilityRollObj && actor) {
            ...
            if (abilityRollObj.isSave) {
                actor.rollAbilitySave(abilityRollObj.ability, rollOptions)
                .then(roll => {
                    abilityRollObj.result = roll._total;
                    socket.emit(SOCKET_EVENTS.FOUNDRY.SEND_ABILITY_ROLL, abilityRollObj);
