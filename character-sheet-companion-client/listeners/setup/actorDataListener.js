...
export async function getAndEmitActorData(socket, actorId, iosSocketId) {
  ...
  function sanitizeActorData(actor5eData) {
    if (actor5eData.actor.hasOwnProperty("actorData")) {
      ...
      if (actor5eData.actor.actorData.hasOwnProperty("details")) {
        actor5eData.actor.actorData.details.appearance = sanitizeHtml(
          actor5eData.actor.actorData.details.appearance
        );
        actor5eData.actor.actorData.details.trait = sanitizeHtml(
          actor5eData.actor.actorData.details.trait
        );
        actor5eData.actor.actorData.details.ideal = sanitizeHtml(
          actor5eData.actor.actorData.details.ideal
        );
        actor5eData.actor.actorData.details.bond = sanitizeHtml(
          actor5eData.actor.actorData.details.bond
        );
        actor5eData.actor.actorData.details.flaw = sanitizeHtml(
          actor5eData.actor.actorData.details.flaw
        );
        if (actor5eData.actor.actorData.details.hasOwnProperty("biography")) {
          actor5eData.actor.actorData.details.biography.value = sanitizeHtml(
            actor5eData.actor.actorData.details.biography.value
          );
          if (
            actor5eData.actor.actorData.details.biography.hasOwnProperty(
              "public"
            )
          ) {
  ...
  function cleanUpActorItems(actor5eData) {
    ...
    let [items, spells, feats] = actor5eData.actor.actorItems.reduce(
      (arr, item) => {
        if (item.type !== "class") {
          imageDataPromises.push(
            toDataURL(item.img).then(
              (imageData) =>
                new Promise((resolve) => {
          ...
          item.data.description.value = sanitizeHtml(
    ...
    imageDataPromises.push(
      toDataURL(actor5eData.actor.img).then(
        (imageData) =>
          new Promise((resolve) => {
