...
export function createAndEmitChatData(socket, userId, actorId, iosSocketId) {
    ...
    if(actor && chatMessages) {
        socket.emit(SOCKET_EVENTS.FOUNDRY.SEND_CHAT_DATA, chatMessages
            .filter(chatMessage => chatMessage.data.type !== 4
                || (chatMessage.data.whisper && chatMessage.data.whisper.includes(userId))
            ...
            .map(chatMessage => {
                ...
                if(chatData.whisper && chatData.whisper.length) {
                    chatMessageToRespond.whisperUsers = chatData.whisper
