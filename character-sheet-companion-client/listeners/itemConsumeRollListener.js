...
export function createAndEmitItemConsumeRoll(socket, itemConsumeRoll, iosSocketId) {
    if(itemConsumeRoll) {
        ...
        if (itemConsumeRollObj && actor && item) {
            ...
            item.rollDamage(rollOptions)
                .then(rollResult => {
                    ...
                    socket.emit(SOCKET_EVENTS.FOUNDRY.SEND_ITEM_CONSUME_ROLL, itemConsumeRollObj, iosSocketId);
                });
        }
    }
}